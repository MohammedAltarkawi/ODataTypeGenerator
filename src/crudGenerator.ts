...
export const generateCrudOperations = (
  parsedMetadata: any,
  typesFile: string,
): { [typeName: string]: string } => {
  const entityTypes =
    parsedMetadata['edmx:Edmx']['edmx:DataServices'][0]['Schema'][0][
      'EntityType'
    ];
  const associations =
    parsedMetadata['edmx:Edmx']['edmx:DataServices'][0]['Schema'][0][
  ...
  entityTypes.forEach((entity: any) => {
    ...
    if (entity['NavigationProperty']) {
      entity['NavigationProperty'].forEach((navProperty: any) => {
        ...
        if (associationEnds) {
          const relatedEntityType = associationEnds.find(
            (end) => end.role === toRole,
          )?.type;
    ...
    if (entity['NavigationProperty']) {
      entity['NavigationProperty'].forEach((navProperty: any) => {
        ...
        if (associationEnds) {
          ...
          const capNavProperty =
            relatedEntityType.charAt(0).toUpperCase() +
            relatedEntityType.slice(1);
...
export const writeCrudToFile = (
  crudOperations: { [typeName: string]: string },
